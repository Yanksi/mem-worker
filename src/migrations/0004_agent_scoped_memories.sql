-- D1 requires a table rebuild to relax a NOT NULL column. Every table that
-- references memories is copied first so dropping the old parent cannot invoke
-- SQLite cascade actions against its audit and graph data.
...
INSERT INTO memories_rebuild (
  id, user_id, agent_id, run_id, actor_id, content, metadata_json, hash, created_at, updated_at, deleted_at
)
SELECT id, user_id, agent_id, run_id, actor_id, content, metadata_json, hash, created_at, updated_at, deleted_at
FROM memories;

INSERT INTO memory_history_rebuild (id, memory_id, operation, content, metadata_json, hash, created_at)
...
SELECT id, user_id, source_entity_id, target_entity_id, relation_type, confidence, evidence_memory_id, metadata_json, created_at, updated_at
FROM relationships;
